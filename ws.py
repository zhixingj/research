...
bb=[[1],[1],[1],[1],[1],[1]]
sfits=np.zeros(solar.szie, tau.size)
...
for i in range(0, tau.size):
    ...
    expA1 = numpy.dot(evec.T, (np.diag(np.exp(eval))).T).T
    expA = numpy.dot(expA1.T, (np.invert(evec)).T).T
    # ;n=expA[0,*]
    n=(expA[0,:])
    ...
    if submain == 1:
        nonzero=where(n != 0 and solarM 1= 0)
