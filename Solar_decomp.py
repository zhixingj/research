...
def solar_decomp(abu_array, amu, iso_zs, fp, ion, massive):
    ...
    masslow = np.arange(numiso, dtype = 'float')
    m_fac = (1 - fp) * abu_array[fe56] / massive[fe56]
    i_fac = fp * abu_array[fe56] / w7[fe56]
    #print(masslow)
    for i in range(0,fe56[0][0] + 1):
        masslow[i] = m_fac[0] * massive[i]
        #print(m_fac[0], massive[i], m_fac[0] * massive[i], masslow[i], i)
        w7[i] = i_fac * w7[i]
...
def test():
    ...
    amuFile = os.path.join(dirname1, 'isomasses_numbers.dat') #file name changed
    amu = np.loadtxt(amuFile, skiprows = 5)

    dirname2 = os.path.dirname(__file__)
    iso_zsFile = os.path.join(dirname2, 'isotope_Zs.dat') 
    iso_zs = np.loadtxt(iso_zsFile)

    dirname3 = os.path.dirname(__file__)
    ionFile = os.path.join(dirname3, 'ions.dat')
    ion = np.loadtxt(ionFile, dtype = 'str')

    dirname4 = os.path.dirname(__file__)
    massiveFile = os.path.join(dirname4, 'massive_2013.dat') # file name changed
    massive = np.loadtxt(massiveFile)

    dirname5 = os.path.dirname(__file__)
    abuFile = os.path.join(dirname5, 'sollo20.dat')
    ...
    resultFile = os.path.join(dirname6, 'result.txt')
    header = 'iso_Zs, amu, sproc, wsproc, rproc, nproc, gproc, w7, massive2, bbn, gcr, novae_priGCR_nu, h_burn'
    np.savetxt(resultFile, result, header=header)
