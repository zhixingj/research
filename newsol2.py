...
f = symbols('f')
k = -0.0110664511265856
Fe56=np.where(uniq == 'fe56')
factorOne = (f * solarM[Fe56][0])/(k)

cuPo = np.where(sion == "cu63")
cuPosition = cuPo[0][0]
init = 1
final = np.empty(uniq.size, dtype = "object")
for i in range(0,cuPosition):
    init = init * 1/(1+(1/(k*sig[i])))
    final[i] = init

cu1 = final[cuPosition-1] * 1/(1/(1-0.2487)+(1/(k*sig[cuPosition])))
