...
def gce():
    ...
    osmium = np.where(element_names == 'os')
    ...
    for i in range(0, numiso):
        for j in range(0,11):
            fraction[i, j] = str(fraction2[i, j]).split()[0]
            # print('frac2', fraction2[i,j],' frac, ', fraction[i,j])
    ...
    #units are mol frac
    # resultFile = 'fraction' + str(date.today()) + '.csv'
    header = 'ion, abundance, sproc, wsproc, rproc, nproc, gproc, w7, massive2, bbn, gcr, novae_priGCR_nu, h_burn'
    # np.savetxt(resultFile, fraction, header=header, delimiter=",")
    np.savetxt(fracFile, content, header = header, delimiter = ',', fmt='%s')
    # dirname = os.path.dirname(__file__)
    # fracFile = os.path.join(dirname, 'fraction_' + str(date.today()) + '.dat')
    # fracContent = ''
    # for i in range(0, numiso):
    ...
    if exportmodel == 1:
        ...
        heavy_s[ba130[0][0] : pb204[0][0]] = data[ba130[0][0] : pb204[0][0], 2]
    ...
    s = np.zeros(numiso)
    scaled = np.zeros((numiso,300))
    sc_em = np.zeros((numiso,300))
    for i in range(0, numiso):
        s[i] = (sollo20[i]-bbn[i])/(1.0-0.0)

    for i in range(0,logz.size):
        for j in range(0, numiso):
             scaled[j,i]=s[j]*(z[i]-1.0)+sollo20[j]

    isoTableFile = os.path.join(dirname, 'Table3_' + str(date.today()) + '.csv')
    table3 = np.empty((numiso, 3), dtype = "object")


    zero22 = np.where(abs(logz + 1) == min(abs(logz + 1)))
    zero33 = np.where(abs(logz + 3) == min(abs(logz + 3)))
    print(scaled[4,zero22[0]])
    # print(logz[zero22],logz[zero33])
    for i in range(0,numiso):
        table3[i,0] = ion[i]
        table3[i,1] = iso[zero22[0],i] / scaled[i,zero22[0]]
        table3[i,2] = iso[zero33[0],i] / scaled[i,zero33[0]]

    header = 'ion, [Z]=-1, Z=-3'
    # np.savetxt(resultFile, fraction, header=header, delimiter=",")
    np.savetxt(isoTableFile, table3, header = header, delimiter = ',', fmt='%s')
    ...
    model = mg_fe_model
    # print(model.dtype)
    dataContent = np.stack((fe_h_model, model), axis = 1)
    dirname = os.path.dirname(__file__)

    dataFile = os.path.join(dirname, "Mg" + str(date.today()) + '.csv')
    # header = 'mgfe, feh, mgfe_sig, feh_sig'
    # np.savetxt(resultFile, fraction, header=header, delimiter=",")
    np.savetxt(dataFile, dataContent, delimiter = ',')
    return "Mg" + str(date.today()) + '.csv'
# gce()

# def saveModel():
#     model = mg_fe_model
#     dataContent = np.stack(fe_h_model, model)
#     dirname = os.path.dirname(__file__)
#
#     dataFile = os.path.join(dirname, "Mg" + str(date.today()) + '.csv')
#     # header = 'mgfe, feh, mgfe_sig, feh_sig'
