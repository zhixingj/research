...
import os
from Solar_decomp import solar_decomp
...
for i in range(0, numiso):
    for j in range(0,11):
        ...
        if fraction[i, j] == '0.0000000':
            fraction[i, j]='\nodata'

#units are mol frac
dirname = os.path.dirname(__file__)
fracFile = os.path.join(dirname, 'fraction_' + date.today() + '.dat')
fracContent = ''
header = 'isotope, &, Solar Abun, &, Main S, &, Weak S, &, R, &, Nu-P, &, Gamma, &, SNIa, &, Massive, &, BBN, &, GCR Spallation, &, Novae/Pri GCR/Nu, &, H-Burn'
np.savetxt(fracFile, fracContent, header = header)
...
if exportmodel == 1:
    ...
    light_s[0 : ba130] = data[0 : ba130, 2]

    heavy_s = np.zeros(numiso)
    heavy_s[ba130 : pb204] = data[ba130 : pb204, 2]

    strong_s = np.zeros(numiso)
    strong_s[pb204 : 287]=data[pb204 : 287, 2]

    strong_s_func = np.zeros(numiso)
    strong_s_func[pb204:bi209] = 1.0
    ...
    fileName = os.path.join(dirname6, 'x_matrix_'+date.today()+'.dat')
...
for i in range(0, z_max):
    ...
    iso_ls[i, 0:ba130] = data[0:ba130, 2] * (z[i])**(ls[SL])
    iso_hs[i, ba130:pb204] = data[ba130:pb204, 2]*(z[i])**(hs[V])
...
for i in range(0, z_max):
    ...
    agba[AGA]=140.
    agbb[AGB]=-0.05
    iso_ss[i, pb204:287] = data[pb204:287, 2]*(np.tanh(agba[AGA]*z[i]+agbb[AGB])+np.tanh(agbb[AGB]))/(np.tanh(agba[AGA]+agbb[AGB])+np.tanh(agbb[AGB]))
...
while i < numiso:
    ...
    element_model[:,j] = np.sum(iso[:, index], axis = 1)
    model_massfrac[:, j] = np.sum(model_massfrac[:, index], axis = 1)
    lodders_massfrac[j] = np.sum(lodders_massfrac[index], axis = 1)
    lsproc[:, j] = np.sum(iso_ls[:, index], axis = 1)
    hsproc[:, j] = np.sum(iso_hs[:, index], axis = 1)
    ssproc[:, j] = np.sum(iso_ss[:, index], axis = 1)
    sproc[:, j] = np.sum(iso_s[:, index], axis = 1)
    rproc[:, j] = np.sum(iso_r[:, index], axis = 1)
    nproc[:, j] = np.sum(iso_nup[:, index], axis = 1)
    gproc[:, j] = np.sum(iso_gam[:, index], axis = 1)
    wsproc[:, j] = np.sum(iso_ws[:, index], axis = 1)
    IIproc[:, j] = np.sum(isoii[:, index], axis = 1)
    Iproc[:, j] = np.sum(iso1A[:, index], axis = 1)
    element_lodders[j] = np.sum(sollo20[index])
    element_model_snii[:, j] = np.sum(isoii[:, index], axis = 1)
    element_model_sn1a[:, j] = np.sum(iso1A[:, index], axis = 1)
...
for i in range(0, z_max):
    ...
    os_fe_model[i] = np.log10((element_model[i, os]/element_lodders[os])/(element_model[i, fe]/element_lodders[fe]))

    ag_fe_model[i] = np.log10((element_model[i, ag,i]/element_lodders[ag])/(element_model[i, fe]/element_lodders[fe]))
    al_fe_model[i] = np.log10((element_model[i, al]/element_lodders[al])/(element_model[i, fe]/element_lodders[fe]))
    ...
    sr_fe_model[i] = np.log10((element_model[i, sr]/element_lodders[sr])/(element_model[i, fe]/element_lodders[fe]))
    zr_fe_model[i] = np.log10((element_model[i, zr]/element_lodders[zr]/(element_model[i, fe]/element_lodders[fe])))
    metallicity[i] = np.log10(np.sum(model_massfrac[i, 2:83] / solarz, axis = 1))
    ...
    sba_h_model[i] = np.log10((sproc[i, ba]/element_lodders[ba])/(element_model(h,i)/element_lodders[h]))
    rba_h_model[i] = np.log10((rproc[i, ba]/element_lodders[ba])/(element_model(h,i)/element_lodders[h]))
    gba_h_model[i] = np.log10((gproc[i, ba]/element_lodders[ba])/(element_model(h,i)/element_lodders[h]))
    ...
    mg_h_model[i] = np.log10((element_model[i,mg]/element_lodders[mg])/(element_model(h,i)/element_lodders[h]))
    mn_h_model[i] = np.log10((element_model[i,mn]/element_lodders[mn])/(element_model(h,i)/element_lodders[h]))
    o_h_model[i] = np.log10((element_model[i,o]/element_lodders[o])/(element_model(h,i)/element_lodders[h]))
    ni_h_model[i] = np.log10((element_model[i, ni]/element_lodders[ni])/(element_model(h,i)/element_lodders[h]))
    ba_h_model[i] = np.log10((element_model[i, ba]/element_lodders[ba])/(element_model(h,i)/element_lodders[h]))
    eu_h_model[i] = np.log10((element_model[i,eu]/element_lodders[eu])/(element_model(h,i)/element_lodders[h]))
